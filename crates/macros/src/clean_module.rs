    ...
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            ...
            match key_text.as_str() {
                ...
                "vm_bindings" | "rust_native_root" | "host_init" | "rust_native_init" => {
                    if legacy_keys.contains(&key_text) {
                        return Err(syn::Error::new_spanned(
                            key,
                            format!("duplicate builtin_module! key `{key_text}`"),
                        ));
                    }
                    legacy_keys.push(key_text);
                    let _: Path = input.parse()?;
                }
                _ => {
                    return Err(syn::Error::new_spanned(
                        key,
                        "expected one of `provider`, `name`, `source`, `exports`, `vm_bindings`, `rust_native_root`, `host_init`, or `rust_native_init`",
                    ));
                }
            }
            if !input.is_empty() {
...
mod tests {
    ...
    #[test]
    fn provider_package_accepts_crate_relative_modules() {
        let args = parse_provider_package(quote! { modules: [window, platform::input] }).unwrap();

        assert_eq!(args.modules.len(), 2);
        assert_eq!(path_segment_tokens(&args.modules[1]), ["platform", "input"]);
    }
