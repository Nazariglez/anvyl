...
macro_rules! spelled_token {
    (
        ...
        impl $name {
            const SPELLINGS: &'static [($name, &'static str)] = &[
...
impl Keyword {
    fn from_str(text: &str) -> Option<Self> {
        Self::SPELLINGS
            .iter()
            .find_map(|(keyword, spelling)| (*spelling == text).then_some(*keyword))
    }
}
...
fn lexer<'src>(
    ...
    choice((
        ...
        line_comment().to(vec![]),
        string_start().ignore_then(string_literal(source, base_offset)),
...
fn string_start<'src>() -> impl Parser<'src, &'src str, (), Extra<'src>> {
    choice((just("\"").ignored(), just("f\"").ignored())).rewind()
}
...
fn map_ident_to_token(s: &str) -> Token {
    Keyword::from_str(s).map_or_else(
        || Token::Ident(ast::Ident(Intern::new(s.to_string()))),
        Token::Keyword,
    )
}
...
fn op<'src>() -> impl Parser<'src, &'src str, Token, Extra<'src>> {
    custom(|input: &mut InputRef<'src, '_, &'src str, Extra<'src>>| {
        let start = input.cursor();
        for (op, spelling) in Op::SPELLINGS {
            let checkpoint = input.save();
            let mut matched = true;
            for expected in spelling.chars() {
                match input.peek() {
                    Some(found) if found == expected => input.skip(),
                    _ => {
                        matched = false;
                        break;
                    }
        ...
        Err(Rich::custom(input.span_since(&start), "expected operator"))
    })
}
...
mod tests {
    ...
    #[test]
    fn operator_spellings_are_longest_first() {
        let mut previous_len = usize::MAX;
        for (_, spelling) in Op::SPELLINGS {
            assert!(
                spelling.len() <= previous_len,
                "operator spellings must stay longest-first: {spelling} after length {previous_len}"
            );
            previous_len = spelling.len();
        }
    }
    ...
    #[test]
    fn f_space_string_is_not_interpolation() {
        assert_eq!(
            tokenize_tokens(r#"f "text""#),
            vec![
                ident_tok("f"),
                Token::Literal(LitToken::String(Intern::new("text".to_string())))
            ]
        );
    }
}
